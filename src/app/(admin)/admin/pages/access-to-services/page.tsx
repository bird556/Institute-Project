...
export default async function AccessToServicesPageHeroEditor() {
  const { data: sections } = await getPageContent('access_to_services')
    ...
    <div className="max-w-2xl mx-auto space-y-6 p-6">
      <div>
        <h1 className="text-2xl font-bold text-[var(--color-text-primary)] dark:text-white">
          Access to Services Page
        </h1>
        <p className="text-sm text-[var(--color-text-muted)] mt-1">
          Edit the heading and subtitle shown at the top of the public Access to Services page.
        </p>
      </div>
      <PageHeroEditor
        page="access_to_services"
