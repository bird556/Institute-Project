...
export async function generateMetadata(): Promise<Metadata> {
  return buildMetadata({ title: 'MA and PhD Theses — Reading List' })
}

export default async function ThesesPage() {
  const [{ data: sections }, supabase] = await Promise.all([
    getPageContent('theses'),
    createClient(),
  ])

  const heroTitle    = sections?.find((s) => s.section === 'hero_title')?.content    ?? 'MA and PhD Theses'
