...
export async function generateMetadata(): Promise<Metadata> {
  return buildMetadata({ title: 'Disabilities — Research' })
}
...
export default async function DisabilitiesPage() {
  ...
  const subtitle = sections?.find((s) => s.section === 'disabilities_description')?.content ?? ''

  const { data } = await supabase
    .from('research_posts')
    .select('id, title, excerpt, cover_path, category, published_at')
    .eq('published', true)
    .eq('category', 'disabilities')
  ...
  const posts = (data ?? []).map((p) => ({
    ...
    cover_url:    p.cover_path
      ...
      : '',
    category:     p.category as 'disabilities',
