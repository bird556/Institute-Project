...
export async function generateMetadata(): Promise<Metadata> {
  return buildMetadata({ title: 'Sexualities — Research' })
}

export default async function SexualitiesPage() {
  const [supabase, { data: sections }] = await Promise.all([
    createClient(),
    getPageContent('research'),
  ])
  const subtitle = sections?.find((s) => s.section === 'sexualities_description')?.content ?? ''
